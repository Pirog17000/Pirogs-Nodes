...
class KSamplerMultiSeedPlus:
    ...
    def sample_integrated(self, model, vae, positive, negative, denoise, steps, cfg, seed_count, seed, 
                         ...
                         vertical_splits=1, horizontal_splits=1, expansion_percent=0.0, 
        ...
        for i in range(input_image.shape[0]):
            ...
            for seed_idx in range(seed_count):
                ...
                for tile_idx, tile in enumerate(tiles):
                    # Store original tile dimensions for scaling back later
                    _, orig_tile_h, orig_tile_w, _ = tile.shape

                    # Supersample tile if enabled
                    if tile_supersampling > 1.0:
                        scaled_tile = self.scale_image_tensor(tile, tile_supersampling, supersampling_min_resolution, supersampling_max_resolution)
                    else:
                        scaled_tile = tile
                    ...
                    concat_latent = vae.encode(scaled_tile)
                    ...
                    # Scale tile back to original size if it was supersampled
                    if tile_image.shape[1] != orig_tile_h or tile_image.shape[2] != orig_tile_w:
                        tile_image = torch.nn.functional.interpolate(
                            tile_image.permute(0, 3, 1, 2),
